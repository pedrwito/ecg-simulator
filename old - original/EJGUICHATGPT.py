...
import ecg_plot  # Add this import at the top
...
def create_ecg_pdf(signal, fs, filename='ecg_plot.png'):
    # Configure ECG plot
    ecg_plot.plot_1(signal, sample_rate=fs, title='ECG Recording')
    
    # Save as PDF with medical ECG styling
    ecg_plot.save_as_png(filename)
    ecg_plot.clear()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        """
        # load signals (OLD)
        self.data = pd.read_csv('coorteeqsrafva.csv', sep=';', header=0, index_col=0)
        self.data = list(self.data['diagnosi'])

        self.signals = np.load('ecgeq-500hzsrfava.npy')

        self.data_unique = set(self.data)
        self.dict_by_rythm = {}
        for rythm in self.data_unique:
            self.dict_by_rythm[rythm] = []

        for i in range(len(self.signals)):
            self.dict_by_rythm[self.data[i]].append(np.array(self.signals[i,:,:]))
...
#        dropdown_lead_label.setContentsMargins(0, 0, 0, 0)
    ...
    def plotSignal(self, dynamic_plot_widget, selected_option_rythm, selected_option_lead, signal_index):
        ...
        export_pdf_button = QPushButton("Exportar como PDF", dynamic_plot_widget)
...
#            symbolBrush="b",
    ...
    def export_to_pdf(self):
        ...
        signal = np.array(self.ECG_SIGNAL)
        # Normalize the signal to fit in the typical ECG range (-2 to 2)
        signal = 2 * (signal - np.min(signal)) / (np.max(signal) - np.min(signal)) - 1
        create_ecg_pdf(signal, self.fs)
